            ...
            if (doc.sendemail) {
                ...
                if (nowTime === doc.sendemailtime) {
                    ...
                    getUsers((users, sys) => {
                        ...
                        if (!isSended && !isNotShareDay && sys.nextshareuserid && sys.sortrules && sys.sortrules.length) {
                            let nextShareUser = _util.getNextShareUserBySys(users, sys);
                            nextShareUser = _util.checkRulesAndReturnUser(nextShareUser, users, sys);
                            //_util.sendEMail(nextShareUser.email, nextShareUser.name, nextShareDate);
                            console.log(nextShareUser);
...
var _util = {
    ...
    getMailText(shareDate) {
        let shareDay = shareDate.getDay() === 0 ? 7 : shareDate.getDay();
    ...
    isNotShareDay(date, sys) {
        ...
        for (let i = 0, len = rules.length; i < len; i++) {
            if (rules[i] === new Date(date).getDay()) {
                flag = false;
                break;
            }
        }
        ...
        if (dates && dates.length) {
            for (let i = 0, len = dates.length; i <= len; i++) {
                for (let j = 0, len1 = rules.length; j <= len1; j++) {
                    if (dates[i] && dates[i].getDay() === rules[j]) {
                        flag = true;
                    }
                }
                if (flag && dates[i]) {
    ...
    getPrevShareDate(sys, date, baseOnShare) {
        ...
        for (let i = 0, len = prev8days.length; i < len; i++) {
            for (let j = 0, len1 = rules.length; j < len1; j++) {
                if (prev8days[i].getDay() === rules[j] && !flag) {
                    flag = true;
    ...
    getNextShareDate(sys, date, baseOnShare) {
        ...
        for (let i = 0, len = next8days.length; i < len; i++) {
            for (let j = 0, len1 = rules.length; j < len1; j++) {
                if (next8days[i].getDay() === rules[j] && !flag) {
                    flag = true;
